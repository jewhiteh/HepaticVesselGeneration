...
private:
	...
	// function to do the final intersection check on the GPU (by pushing branches)
	int pushBranchesOnGPU(float xstart[], float ystart[], float zstart[], float xstop[], float ystop[], float zstop[], int root[],
		int Qs[], int section[], int *count, float Qscale, float gamma_radii, float scale, int terminal_pts, int children[], int arrayAlloc, float r[]);
