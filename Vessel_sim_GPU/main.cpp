...
int main() {
	...
	for (int i = 1; i <= tree_number; i++) {
		...
		mt.seed(seed);
#endif
		string dir = "../Vessel_sim_GPU/CLines/seed_" + to_string(i) + ".txt";
		ofstream file;
		file.open(dir);
		file << seed;
		file.close();
...
void Liver::create_tree(float xstart[], float ystart[], float zstart[], float xstop[], float ystop[], float zstop[], int root[], int section[],
	...
	int gpuSwitch = 5000; //when to switch to GPU processing for cost calculation
	int gpuSwitch2 = 1200; //when to switch to gpu processing for intersection check
	if (gpuSwitch < gpuSwitch) { cerr << "error in method create_tree: GPU switch 1 needs to be greater than GPU switch 2" << endl; }
	...
	bool intial = 1;//initialize the GPU memcopy
	bool intialIntersection = 1;//initialize the GPU memcopy for interesction check
	...
	// Specify texture memory object
	struct cudaResourceDesc resDesc;
	memset(&resDesc, 0, sizeof(resDesc));
	resDesc.resType = cudaResourceTypeLinear;
	resDesc.res.linear.desc = channelDesc;
	resDesc.res.linear.sizeInBytes = sizeof(float) * arrayAlloc;
	...
	//dev_root: Allocate CUDA array in device memory
	float *cuArrayRoot;
	cudaMalloc((void**)&cuArrayRoot, sizeof(float) * arrayAlloc);
	resDesc.res.linear.devPtr = cuArrayRoot;

	// Create texture object
	cudaTextureObject_t texObjRoot = 0;
	cudaStatus = cudaCreateTextureObject(&texObjRoot, &resDesc, &texDesc, NULL);

	//dev_V
	float *cuArrayV;
	cudaMalloc((void**)&cuArrayV, sizeof(float) * arrayAlloc);
	channelDesc = cudaCreateChannelDesc(32, 0, 0, 0, cudaChannelFormatKindFloat);
	resDesc.res.linear.desc = channelDesc;
	resDesc.res.linear.devPtr = cuArrayV;
	cudaTextureObject_t texObjV = 0;
	cudaStatus = cudaCreateTextureObject(&texObjV, &resDesc, &texDesc, NULL); if (cudaStatus != cudaSuccess) { goto Error1; }

	//allocate memory for all device arrays
	cudaStatus = cudaMalloc((void**)&dev_xstart, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&outCost, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_ystart, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_zstart, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_xstop, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_ystop, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_zstop, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_section, sizeof(int) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_V, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_Q, sizeof(int) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&foundIntersection, sizeof(bool)); if (cudaStatus != cudaSuccess) { goto Error1; }
#endif
	...
	for (int n = 0; n < terminal_pts + offset + removed; n++) {
		...
		int best_idx = -1;
#if GPU
		//check if it is faster to run on the CPU or GPU
		if (n < gpuSwitch) {
#endif
...
#pragma omp parallel for
		...
		//send to GPU
			...
			//calc cost on GPU
			best_idx = GetBestCost(dev_xstart, dev_xstop, dev_ystart, dev_ystop, dev_zstart, dev_zstop,
				dev_Q, texObjV, dev_section, texObjRoot, point[0], point[1], point[2], sctn, qPower, count, outCost);
			cudaStatus = cudaMemcpy(all_costs, outCost, sizeof(float) * arrayAlloc, cudaMemcpyDeviceToHost); if (cudaStatus != cudaSuccess) { goto Error2; }
		}
#endif
...
#if GPU
				//check if it would be faster to do it on the GPU
				if (n < gpuSwitch2) {
#endif
...
#pragma omp parallel for shared(flag)
				...
				//check for interesction on the GPU
				else {
					//copy check to GPU
					bool check = 0;
					cudaStatus = cudaMemcpy(foundIntersection, &check, sizeof(bool), cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }

					//check for intersection on GPU
					checkIntersectionGPU(dev_xstart, dev_xstop, dev_ystart, dev_ystop, dev_zstart, dev_zstop, dev_Q, xcentroid,
						ycentroid, zcentroid, r_con, r_pos1, r_pos2, gamma, point[0], point[1], point[2], count, root[pos], Qscale, scale, sister_branch,
						xstart[pos], ystart[pos], zstart[pos], xstop[pos], ystop[pos], zstop[pos], pos, children[2 * pos], children[2 * pos + 1], foundIntersection);

					//get value from GPU
					cudaStatus = cudaMemcpy(&check, foundIntersection, sizeof(bool), cudaMemcpyDeviceToHost); if (cudaStatus != cudaSuccess) { goto Error2; }

					//there is an intersection, set cost to infinity
					if (check) {
						curBestCost = INFINITY; }
				}
#endif
#endif
		...
		if (best_cost < INT_MAX) {
			...
			if (n + 1 >= gpuSwitch) {
				...
				if (intial) {
					//copy memory
					cudaStatus = cudaMemcpy(dev_xstart, xstart, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_ystart, ystart, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_zstart, zstart, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_xstop, xstop, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_ystop, ystop, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_zstop, zstop, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_Q, Qs, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_section, section, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(cuArrayRoot, root, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(cuArrayV, V, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					intial = 0;
				}
#if !Intersection_check
				//copy changed memory
			...
			if (n + 1 >= gpuSwitch2) {
				...
				if (intialIntersection) {
					...
					cudaStatus = cudaMemcpy(dev_zstop, zstop, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_Q, Qs, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					intialIntersection = 0;
				}
				//copy changed memory
				else {
					//GPU kernel to update the device arrays
					updateGPUmemory(dev_xstart, dev_xstop, dev_ystart, dev_ystop, dev_zstart, dev_zstop, count, best_idx, xcentroid, ycentroid, zcentroid, point[0], point[1], point[2], dev_section, sctn);
					cudaStatus = cudaMemcpy(cuArrayV, V, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(dev_Q, Qs, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
					cudaStatus = cudaMemcpy(cuArrayRoot, root, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
				}
...
#if GPU
	//Error detection for GPU memory allocation and copy
	Error1 :
	   ...
	   Error:
		   ...
		   cudaFree(dev_Q);
		   cudaFree(dev_length);
		   cudaFree(dev_section);
		   cudaFree(foundIntersection);
		   return;
#endif
}
...
int  Liver::build_tree(const int Nx, const int Ny, const int Nz, int terminal_pts, int tree_number,
	...
	string fname2 = "../Vessel_sim_GPU/BloodDemandMap/Phantom.dat";
	FILE *fid; 
	errno_t err;
...
#if GPU && !constant_seed
	int check = pushBranchesOnGPU(xstart, ystart, zstart, xstop, ystop, zstop, root, Qs, section, &count, Qscale, gamma, scale, terminal_pts, children, arrayAlloc, r);
#else
	...
	*/
	...
	string dir2 = "../Vessel_sim_GPU/CLines/Tree" + to_string(tree_number) + ".txt";
	ofstream tree;
	tree.open(dir2);
	...
	dir2 = "../Vessel_sim_GPU/CLines/Radii" + to_string(tree_number) + ".txt";
	ofstream rad;
	rad.open(dir2);
	for (unsigned int t = 0; t < prePushCount; t++) {
		rad << r[t] << endl;
...
//Function to move the small branches out of the way if they cause an intersection on the GPU
// Input: Tree object with arrays
//	x,y,z start/stop: x,y,z start/stop points of all branches
//	root: list of all parents for each branch
//	Qs: perfusion of each branch
...
//	r: radius of each branch
// Output: Tree with branches pushed to not cause intersection
int Liver::pushBranchesOnGPU(float xstart[], float ystart[], float zstart[], float xstop[], float ystop[], float zstop[], int root[],
	int Qs[], int section[], int *count, float Qscale, float gamma, float scale, int terminal_pts, int children[], int arrayAlloc, float r[]) {
	//variables 
	queue<int> recheck;//branches to recheck
	int max_indx = *count;
	int maxCheck = 1000;
	...
	int index = -1;
	int *dev_root;
	cudaError_t cudaStatus = cudaMalloc((void**)&dev_root, sizeof(int) * arrayAlloc);
	...
	resDesc.resType = cudaResourceTypeLinear;
	resDesc.res.linear.desc = channelDesc;
	resDesc.res.linear.sizeInBytes = sizeof(int) * arrayAlloc;
	...
	texDesc.readMode = cudaReadModeElementType;
	texDesc.normalizedCoords = 0;
	resDesc.res.linear.devPtr = dev_root;
	...
	cudaStatus = cudaMalloc((void**)&dev_section, sizeof(int) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_Q, sizeof(int) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_r, sizeof(float) * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_children, sizeof(int) * 2 * arrayAlloc); if (cudaStatus != cudaSuccess) { goto Error1; }
	cudaStatus = cudaMalloc((void**)&dev_index, sizeof(int)); if (cudaStatus != cudaSuccess) { goto Error1; }

	//copy memory to all device arrays
	cudaStatus = cudaMemcpy(dev_xstart, xstart, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	cudaStatus = cudaMemcpy(dev_ystart, ystart, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	cudaStatus = cudaMemcpy(dev_zstart, zstart, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	cudaStatus = cudaMemcpy(dev_xstop, xstop, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	...
	cudaStatus = cudaMemcpy(dev_zstop, zstop, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	cudaStatus = cudaMemcpy(dev_Q, Qs, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	cudaStatus = cudaMemcpy(dev_r, r, sizeof(float) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	cudaStatus = cudaMemcpy(dev_section, section, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	cudaStatus = cudaMemcpy(dev_children, children, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	cudaStatus = cudaMemcpy(dev_root, root, sizeof(int) * arrayAlloc, cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }


	//copy index to GPU
	cudaStatus = cudaMemcpy(dev_index, &index, sizeof(int), cudaMemcpyHostToDevice); if (cudaStatus != cudaSuccess) { goto Error2; }
	...
	for (int j = 0; j < max_indx; j++) {
		...
		float r_j = r[j];//radius of j branch
		pushBranchesGPU(dev_xstart, dev_xstop, dev_ystart, dev_ystop, dev_zstart, dev_zstop, texObjRoot, dev_Q,
			dev_section, max_indx, Qscale, gamma, scale, terminal_pts, dev_children, dev_index, xstart[j], ystart[j], zstart[j],
			xstop[j], ystop[j], zstop[j], r[j], dev_r, j, sister, root[j], 0);

		//get value from GPU
		cudaStatus = cudaMemcpy(&index, dev_index, sizeof(int), cudaMemcpyDeviceToHost); if (cudaStatus != cudaSuccess) { goto Error2; }
		//there was an intersection found
		if (index != -1) {
			//Determine which branch is smaller so we can move it
			float closest[3], mov_vec[3];//which closest point we are moving
			int indx;//branch indx that we are moving
			int k = index;//branch where an intersection was found
			float c1[3], c2[3];//closest points on branch one and two
			float d = dist3D_Segment_to_Segment(xstart[j], ystart[j], zstart[j], xstop[j], ystop[j], zstop[j],
				xstart[k], ystart[k], zstart[k], xstop[k], ystop[k], zstop[k], c1, c2);
			if (d == 0) { cerr << "Error line: 1622, possible division by 0." << endl; }
			//move j branch
			if (r[j] < r[k]) {
				for (int ind = 0; ind < 3; ind++) {
					closest[ind] = c1[ind];
					mov_vec[ind] = (c1[ind] - c2[ind]) * (.001 + r[j] + r[k]) / d;//.005 is a small safety factor for rounding errors
				}
				indx = j;
			}
			//move k branch
				...
				for (int ind = 0; ind < 3; ind++) {
					closest[ind] = c2[ind];
					mov_vec[ind] = (c2[ind] - c1[ind]) * (.001 + r[j] + r[k]) / d;//.005 is a small safety factor for rounding errors
				}
				indx = k;
			}
			//See if we are on the start of the branch
	...
	//Due the rechecks
	maxCheck = recheck.size() * 10;//breaks out if the recheck is stuck in a loop
	while (!recheck.empty()) {
		if (recheck.size() > maxCheck) { return -1; }
		int j = recheck.front();
		recheck.pop();
		...
		pushBranchesGPU(dev_xstart, dev_xstop, dev_ystart, dev_ystop, dev_zstart, dev_zstop, texObjRoot, dev_Q,
			dev_section, *count, Qscale, gamma, scale, terminal_pts, dev_children, dev_index, xstart[j], ystart[j], zstart[j],
			xstop[j], ystop[j], zstop[j], r[j], dev_r, j, sister, root[j], 1);
		...
		if (index != -1) {
			...
			float d = dist3D_Segment_to_Segment(xstart[j], ystart[j], zstart[j], xstop[j], ystop[j], zstop[j],
				xstart[k], ystart[k], zstart[k], xstop[k], ystop[k], zstop[k], c1, c2);
			if (d == 0) { cerr << "Error line: 1771, possible division by 0." << endl; }
			//move j branch
			if (r_j < r[k]) {
				for (int ind = 0; ind < 3; ind++) {
					closest[ind] = c1[ind];
					mov_vec[ind] = (c1[ind] - c2[ind]) * (.001 + r_j + r[k]) / d;//.005 is a small safety factor for rounding errors
				}
				indx = j;
			}
			//move k branch
				...
				for (int ind = 0; ind < 3; ind++) {
					closest[ind] = c2[ind];
					mov_vec[ind] = (c2[ind] - c1[ind]) * (.001 + r_j + r[k]) / d;//.005 is a small safety factor for rounding errors
				}
				indx = k;
			}
			//recheck these branches
			recheck.push(j);
			recheck.push(k);
			//See if we are on the start of the branch
			int parent = root[indx];
